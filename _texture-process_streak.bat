del /f "%~dp0_ark\ui\track\streak\streak.dta"
...
move "%~dp0custom_textures\streak\streak.dta" "%~dp0_ark\ui\track\streak\streak.dta"
...
FOR /F "tokens=*" %%G IN ('dir /b *.png') DO "%~dp0dependencies/superfreq.exe" png2tex "%~dp0custom_textures\streak/%%G" "%~dp0_ark\ui\track\streak\gen\%%~nG.png_xbox" --platform x360 --miloVersion 25
cd "%~dp0_ark/ui/track/streak/gen"
FOR /F "tokens=*" %%G IN ('dir /b *.png_xbox') DO python "%~dp0dependencies/swap_rb_art_bytes.py" "%~dp0_ark/ui/track/streak/gen/%%G" "%~dp0_ark/ui/track/streak/gen/%%~nG.png_ps3"